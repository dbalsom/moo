...
impl PartialEq for MooRegisters1 {
...
impl From<&MooRegisters1Init> for MooRegisters1 {
    fn from(init: &MooRegisters1Init) -> Self {
        Self {
...
impl From<(&MooRegisters1Init, &MooRegisters1Init)> for MooRegisters1 {
    fn from(init: (&MooRegisters1Init, &MooRegisters1Init)) -> Self {
...
pub struct MooRegisters1Printer<'a> {
    ...
    pub diff: Option<&'a MooRegisters1>,
}
...
impl Display for MooRegisters1Printer<'_> {
    #[rustfmt::skip]
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let c_chr = if MooRegisters1::FLAG_CARRY & f != 0 { 'C' } else { 'c' };
        let p_chr = if MooRegisters1::FLAG_PARITY & f != 0 { 'P' } else { 'p' };
        let a_chr = if MooRegisters1::FLAG_AUX_CARRY & f != 0 {
            'A'
        } else {
            'a'
        };
        let z_chr = if MooRegisters1::FLAG_ZERO & f != 0 { 'Z' } else { 'z' };
        let s_chr = if MooRegisters1::FLAG_SIGN & f != 0 { 'S' } else { 's' };
        let t_chr = if MooRegisters1::FLAG_TRAP & f != 0 { 'T' } else { 't' };
        let i_chr = if MooRegisters1::FLAG_INT_ENABLE & f != 0 {
            'I'
        } else {
            'i'
        };
        let d_chr = if MooRegisters1::FLAG_DIRECTION & f != 0 {
            'D'
        } else {
            'd'
        };
        let o_chr = if MooRegisters1::FLAG_OVERFLOW & f != 0 {
            'O'
        } else {
            'o'
        };
        let m_chr = if self.cpu_type.is_intel() {
            ...
            if f & MooRegisters1::FLAG_MODE != 0 {
                'M'
            } else {
                'm'
            }
        ...
        let nt_chr = if f & MooRegisters1::FLAG_NT != 0 { '1' } else { '0' };
        let iopl0_chr = if f & MooRegisters1::FLAG_IOPL0 != 0 { '1' } else { '0' };
        let iopl1_chr = if f & MooRegisters1::FLAG_IOPL1 != 0 { '1' } else { '0' };
